...
trait_list <- out_data_full %>%
    ...
    dplyr::filter(originalname.outcome == "Alzheimers") %>%
    # dplyr::filter(originalname.outcome == "Parkinson's disease") %>%
    # dplyr::filter(originalname.outcome == "Cerebrospinal fluid p-tau levels" |
...
for (i in trait_list) {
    ...
    print(i)
    out_data <- out_data_full %>% dplyr::filter(originalname.outcome == i)

    out_data <- out_data %>% dplyr::rename(
    ...
    out_data <- out_data %>% dplyr::filter(SNP %in% snplist)
    p_data <- p_data %>% dplyr::filter(rsids %in% out_data[, (SNP)])
    ...
    # dplyr::filter out rows where alleles are not the same
    out_data <- out_data %>%
        dplyr::filter((Allele1 == p_data$effectAllele &
    ...
    out_data <- out_data %>%
        ...
        dplyr::select(-temp_Allele1, -temp_Allele2, -flip)
    ...
    # dplyr::select rsids
    ldmat <- ldmat[p_data$rsids, p_data$rsids]
    ...
    # )
    # SNP_hit <- significant_snps %>% pull(SNP)

    plot_dataset(D1,
        highlight_list = pqtl,
        main = paste("Exposure", protein_name, sep = " ")
    )
    plot_dataset(D2,
        highlight_list = pqtl,
        main = paste("Outcome", protein_name, sep = " ")
    )
    ...
    # S1 <- tryCatch(
    #     {
    #         runsusie(D1, maxit = 1000, coverage = 0.1) # low coverage allowing weaker signals
    #     },
    #     error = function(e) {
    ...
    # S2 <- tryCatch(
    #     {
    #         runsusie(D2, maxit = 1000, coverage = 0.1) # low coverage allowing weaker signals
    #     },
    #     error = function(e) {
...
# my.res$results$SNP.PP.H4 > (1 - 05)

# Pal <- colorRampPalette(c("white", "blue"))
# Col <- Pal(100)[ceiling(100 * my.res$results$SNP.PP.H4)]

# ceiling(100 * my.res$results$SNP.PP.H4)
# +
#     ggsave(file = paste0(
#         "./results/",
#         out_data$trait_short[1], "_",
#         protein_name, "_coloc.png"
#     ))
# library(ggplot2)
